...
export const getArticle = async ({ commit, dispatch, rootState }) => {
...
export const saveArticle = async ({commit, state, rootState}, payload) => {
  ...
  const { ok, response, error } = await sureThing(HTTP.patch(route, p,
    {
      headers: {
        Authorization: `Bearer ${rootState.authentication.token}`
      }
  ...
  ok
    ? commit(types.SET_STATUS, { message: !skip ? 'The classification has been saved' : 'The text has been skipped', data: response })
    : commit(types.SET_ERROR, error)
}
