...
export const setFilters = async ({ commit, dispatch }, payload) => {
  commit(types.SET_FILTERS, payload, (err) => { console.log(err) })
  dispatch('searchAll')
  dispatch('getAggregations', payload.type)
}

export const setIndices = async ({ commit, dispatch }, payload) => {
  commit(types.SET_INDICES, payload, (err) => { console.log(err) })
  dispatch('searchAll')
...
function setRoute (query, type = null, filters = null, page = 1, aggs = false, limit = 10) {
  ...
  if (type) {
    ...
    if (filters[t] && filters[t].length > 0) string += `&f=${filters[t].join(',')}`
  }

  if (filters.field && filters.field.length > 0) string += `&field=${filters.field}`
